# Method -- Test (iii') Bifurcation-Mix Conjecture

This document describes the classification rule, the roster, and the test statistics for
test (iii'). The floor is **pi_B = 0.60**: the conjecture is REFUTED on this roster if the
...
each event on a labelled roster as B / N / R by a declared rule, blind to the
...
- **Wikipedia (n = 14 OK events)** -- `validation/wikipedia/roster.py`,
  ...
  Robinhood, Silicon Valley Bank, Terra -- carry no blocks and no collapse number,
...
**Combined roster size = 24 labelled cascades** (14 Wikipedia + 10 WSB). This meets
the M = 20 minimum. Both rosters are CONVENIENCE SAMPLES of attention cascades, not
...
**Existing-community share f_existing** (identical definition on both substrates):
fraction of ONSET-window focal activity made by participants who were ALSO active in the
pre-onset window (i.e. members of the pre-onset partition / pre-onset author set).
- Wikipedia: onset-window = `[onset, onset+21d)` focal article revisions; pre-onset set =
  the K-block editor partition. (Exactly V1's `f_existing`.)
...
**Onset abruptness a_abrupt**: the ratio of onset-window mean daily
...
- `SHARE_HI = 0.20` -- existing-share at or above this counts as "high existing-share"
  ...
  Zelenskyy 0.27). 0.20 is the natural gap between the two clusters, and is set before the
  WSB shares are computed.
...
Rationales are written into the script's `SUBSTANTIVE` table, one line each.
...
- **Pass/fail vs pi_B:** SUPPORTED if B-fraction >= 0.60; REFUTED if < 0.60. Reported
  for each classification separately; if they disagree across the floor, that is
  reported as the headline.

This is a proportion test against a floor, not a skill test; no null model is required.
