...
Thresholds: drift bound **X = 15%**, composition churn floor **Y = 40%**.
...
from the rest of the internet. The fix is to measure a BASKET of
...
(see `FETCH_LOG.md`). Basket: wallstreetbets, investing, stocks,
...
- **Total attention drift = +1,334%** (the basket total grew about 14x). The
  bound is X = 15%. The total did not stay flat; it ballooned by
...
Under the pass/fail rule for test (i), "net attention-minutes expand
...
2. **A basket is not the economy.** This finance/meme basket is a tiny slice of the
   ...
   **at the ecosystem-basket scale** and leaves the global claim untested - it cannot
   be settled until the measurement spans the top-N platform total.
...
mania - it behaves like the single subreddit did, just with more channels. The
