...
|---|---|---|
| **Dynamic N_eff collapse — community-specificity** (`validation/neff_v4/`) | **9 of 12** r/wallstreetbets cascades collapse past their own block-label shuffle null on a fresh pre-registered roster; the median cascade beats all 300 reshuffles. The raw-magnitude reading is reported, separately, as non-discriminating. | **SIGNIFICANCE WITHDRAWN (2026-07-25).** The counts stand; the binomial *p* = 1.7×10⁻⁷ does not. It assumed a null fire rate p0 = 0.10 that this repository's own quiet windows contradict (10/12 quiet windows fire; CP95 lower bound 0.56 against a 0.378 break point), so the frozen rule's condition (b) fails at every threshold the run itself measured. Open item: measure p0 over ≥30 quiet pseudo-onsets. Audit: [`validation/neff_v4/P0_AUDIT.md`](validation/neff_v4/P0_AUDIT.md). |
