...
### The LOGOS companion paper (v0.1, new)

`logos.tex` applies the same discipline to a different object: the **machines** rather than the societies. It specifies a 10T+ "Mixture-of-Towers" architecture from published 2024–2026 components, then audits it adversarially. **No model in it has been trained and no system has been served** — it is a specification, an arithmetic audit, and a gap ledger. Its status is worse than psychohistory's, not better: psychohistory has one sealed pre-registered pass, LOGOS has zero runs.

The connection is not decorative. A LOGOS-class system executing autonomously over long horizons is precisely the **major player** and **out-of-model agent** that psychohistory names as its own unrepairable failure mode. And the Mixture-of-Towers is the same near-decomposability bet about a machine that psychohistory makes about a society, with the same failure mode when inter-unit coupling grows. Running the other way, `logos-harness` — the bootstrap strategy for when human text runs out — needs an adjudicator the model cannot fake, and psychohistory's validation pipeline is one; while psychohistory's blocked forward-forecast falsifiers need a trajectory generator, which is what the harness is. **They are the same missing piece approached from two directions.**
...
| `logos.tex` | **The LOGOS paper** — *System Design, Theoretical Limits, and Regulatory Structure of 10T+ Mixture-of-Towers Architectures*. A companion position paper on the machines rather than the societies: a 5×2.8T Mixture-of-Towers specification (BAR modular post-training, heterogeneous + latent MoE routing, KDA/MLA hybrid attention, MXFP4 serving, multi-tenant adapters, P2P decode), with its arithmetic audited and its joints named. Its three original contributions are the **dense/sparse category error** (the usual "10T is impossible" argument prices a dense model nobody is building), the **compute-threshold arbitrage** (the EU AI Act's 10²⁵ FLOP presumption is written against *a model*, and modular composition has no settled answer to "how many models is this"), and the **bootstrap harness** for what happens when tokens run out. |
| `logos/` | The LOGOS companion directory: [`ARCHITECTURE_REVIEW.md`](logos/ARCHITECTURE_REVIEW.md) (14 findings, 3 critical — the adversarial referee report on the architecture), [`BIBLIOGRAPHY_REVIEW.md`](logos/BIBLIOGRAPHY_REVIEW.md) (every citation checked against a primary source: 21 verified, 4 corrected, 4 downgraded to blog-grade sourcing, 3 dropped as unsourceable, 11 missing attributions supplied), [`GAPS.md`](logos/GAPS.md) (the work ledger, graded by *how much GPU* — including four CPU checks withdrawn as circular), and [`LOGOS_HARNESS.md`](logos/LOGOS_HARNESS.md) (**`logos-harness`** — the agentic bootstrap spec: towers disagree, act on the world, and the world adjudicates; tested on Pokémon for volume and on **this repository's own psychohistory pipeline** for validity). |
