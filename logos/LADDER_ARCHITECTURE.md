...
4. **The over-constraint is a sensitivity analysis, not a result, and this document previously called it "the sharpest result in the programme". That does not hold.** `GAPS.md` §4a and `../logos.tex` §3.3 establish `f ≤ 1-g`, hence a unique-corpus requirement **bounded above** by `5.6e13 * (5 - 4g)`, which is **2.8e14 at `g = 0`** and **6.05e13 at `g = 0.98`** against a central supply estimate near **6e13**. Three qualifiers were dropped every time that sentence was restated in this file, and the paper (`../logos.tex` §3.3) carries all three: the bound is an **upper** bound, so `g >= 0.98` *suffices* and is not *needed* (`g = 0.20` with `f = 0.02` satisfies both constraints); the verdict is set by a point estimate inside a factor-of-ten interval, running from unsatisfiable at any `g` to `g* = 0.357`; and it is not improved by correcting the token law. **What survives is not a statement about `g` at all**: it is **X-04 at MEDIUM**, that a corpus overlap high enough to meet the central supply estimate is a corpus overlap high enough to cost the diversity claim, which `../logos.tex:719` states without `g`. §1.5 carries the sensitivity table. An earlier draft of this document said `f` was "unbounded anywhere in the paper"; that is false as of `../logos.tex` v0.3 and is superseded wherever it appeared.

There are now **fourteen measurements** in `../logos.tex` §15. **Seven run on one consumer accelerator** (F3, F4, F9, F10 as independent experiments; F5 and F13 limb (b) falling out of F9; F14 sharing an instrument with F13 limb (a), which runs on that card too but not as an F9 arm). **F11 and F12 need no accelerator at all.** F14 is new in v0.3 and it is the residue of L-01.
...
### 1.5 The over-constraint: a sensitivity analysis, and what it does and does not decide

**Status: DOWNGRADED. This section previously read "no setting of `g` works" and called it "the sharpest result in the programme". Both are dropped here rather than softened elsewhere.** The arithmetic below is `logos.tex` §3.3's, collected in `GAPS.md` §4a, and it is a sensitivity analysis over an unmeasured supply figure. It constrains what the ladder can claim; it does not by itself decide what the ladder is allowed to be.
...
**`f` is not a free parameter over `[0,1]`. Fixing the training plan fixes `g`, and `g` bounds `f` without anybody touching a corpus.** That is a real improvement on the state of the review: an earlier draft of this document, and X-04 recorded `f` as unbounded anywhere in the paper. **It no longer is, and that claim is superseded wherever it appeared here.**

Evaluated against the central unique-token-stock estimate of about `6e13` that `logos.tex` §11.4 reads Eq. (residency) against:

| `g` | **Worst-case** bound on unique-corpus requirement | Against a central supply near 6e13 | Lineage |
|---:|---:|---|---|
...
**Read the second column as the worst case it is.** `f <= 1-g` is an upper bound, so the row at `g = 0.50` says the requirement is *somewhere between* `5.6e13` and `1.68e14`, set by a corpus-overlap quantity the row does not carry. An earlier version of this section read the column as the value and concluded that the data wall "**needs** `g` at or above about 0.98" and that "below that the token argument for towers is dead". **Both sentences are dropped. `g >= 0.98` guarantees the data wall is met; below it the outcome is set by the corpus and not by the schedule.** The counterexample is one line: `g = 0.20` with `f = 0.02` satisfies `f <= 1-g` and meets the supply estimate, while leaving 80 percent of every tower's budget post-branch, which is more divergence than the `g = 0.50` row this file previously called "genuinely divergent".

**The three demands, restated without the quantifier error.**

- **X-04, the data wall.** Eq. (residency) meets a supply of `S` when `f <= (S/5.6e13 - 1)/4`, which at `S = 6e13` is `f <= 0.02`. High `g` is *one sufficient route* to a low `f`; a corpus that is 98 percent shared core is another, and neither requires the other.
- **C-02 and the diversity conjecture (F13, F14)** need overlap **low**. At 98 percent shared corpus, whether the sharing arrives through the seed or through the core, §3.2's `rho` table prices most or all of the ensemble gain as lost. The conjecture is stated at `λ = 0` and F13's limb (a) explicitly rules out two finetunes of one checkpoint as an instrument.
...
**The conflict that survives, and its severity.** It is not about `g`. Stated in `f`: a corpus overlap high enough to meet the central supply estimate is a corpus overlap high enough to cost the diversity claim. `logos.tex:719` states exactly that without mentioning `g`, and **it is already on the books as X-04 at MEDIUM**. §3.3's contribution is an upper bound on `f`, while the binding constraint on the diversity side is a *lower* bound on `f`, about which `f <= 1-g` says nothing at all. This section is graded at X-04's severity until a supply measurement exists.

**Sensitivity, which is what actually decides the verdict.** Write `S` for unique supply and `r` for tokens per total parameter; the `g` at which the requirement's worst case meets supply is `g* = (5 - S/(r * 2.8e12))/4`.

| `S` at `r = 20` | `S/(rN)` | `g*` |
|---:|---:|---|
| `2e13` (90% lower) | 0.357 | **1.161, unsatisfiable at any `g`** |
| `6e13` (central) | 1.071 | 0.982 |
| `1e14` | 1.786 | 0.804 |
| `2e14` (90% upper) | 3.571 | 0.357 |

Across the survey's own 90 percent interval the verdict runs from "the data wall blocks the architecture at every setting, decomposed or not" to "any branch consuming more than 36 percent of the token budget is fine". **Nothing in this repository measures that number.**

**Two inputs, reported plainly: one was suspected here and is sound, and the other is sound and hurts.**

- **The supply figure is faithful to its primary source.** Villalobos et al. put the effective stock at 300T tokens with a 90 percent interval of 100T to 1000T, embedding a 5x epoch cap that the source reduces to 5x explicitly, which backs out to a `6e13` central unique stock with a `2e13` to `2e14` interval. The `logos.tex:94` premise reproduces that exactly, interval included. It is a survey figure and it is correctly used.
- **Correcting `D_opt = 20N` hurts rather than helps.** At the measured token-per-total-parameter ratios of two published trillion-scale sparse models, Kimi K2 at 14.9 (`arXiv:2507.20534`) and DeepSeek-V3 at 22.1 (`arXiv:2412.19437`), `g*` at the central estimate moves to **0.890** and to **1.008**, the second unsatisfiable at any `g`. So the escape route through F1 is not an escape route.

**The branch on which the diversity claim survives, and what it rests on.** Against the 90-percent **upper** supply bound of `2e14`, Eq. (residency) survives to `f ≈ 0.64`, which admits a genuinely divergent branch and leaves the diversity claim intact. That is a statement about which token-supply estimate is believed, not a theorem about architectures, and it means this document's most load-bearing constraint is inherited from a survey figure rather than derived.
...
**Response 1 was adopted here on the superseded premise, and it is re-examined rather than left standing.** The original justification was three-legged: that the trilemma forces a corner, that `logos.tex` §3.3 resolves L-01 against the diversity claim, and that this document's own §3.4 independently predicts the ladder's alignment objective pushes `rho_bar` the wrong way. **The first leg is gone.** With the bound read correctly and the supply figure read as an interval, the arithmetic does not force a corner: at the 90-percent upper supply bound response 2 is available at any `g >= 0.36`, and at the central estimate the outcome is decided by corpus overlap, which nobody has measured. So the choice below is a **judgement under an unmeasured parameter**, and it must not be quoted as arithmetic.

**The decision stands, on the two remaining legs, and it is weaker than it was.** Response 1 is retained because `logos.tex` §3.3 resolves L-01 against the diversity claim on grounds that have nothing to do with the supply estimate (Branch-Adapt-Route needs `λ >= 1` and there is no version of this architecture without it), and because §3.4 predicts the alignment objective decorrelates the wrong way. **Three things that now change.**

- **Response 2 is no longer excluded by arithmetic.** It is excluded by a judgement about which supply estimate to believe, and a reader who believes the upper bound should read §3, §7 and §10 at different weights, per §12 item 10.
- **What is dropped is the diversity budget from the *justification*, not the diversity *question*.** §7 still tests it, and after this downgrade §7 carries more of the load rather than less, because the arithmetic no longer pre-empts it.
- **Response 3, disputing the supply estimate, is not a fallback but the live option**, since everything above turns on `6e13` and §10 is the one mechanism in this programme that would move it.

**Response 1 is a real loss and it should be written as one:** it removes the third motivation of `logos.tex` §3.1, which that section itself calls "the one we think matters most". Removing it on a judgement rather than on a proof is a worse position to be in than the previous revision of this section claimed.
...
- Fort, Hu and Lakshminarayanan (arXiv:1912.02757), verbatim: "random initializations explore entirely different modes, while functions along an optimization trajectory or sampled from the subspace thereof cluster within a single mode predictions-wise", and **"the decorrelation power of random initializations is unmatched by popular subspace sampling methods."** Ensemble benefit is a function of error decorrelation; decorrelation comes from independent initialisation; a ladder distilled from one parent on one corpus is the precise opposite of independent initialisation. Recorded: the diversity-accuracy-plane numerics were not extracted, so only the two qualitative statements above are used.
...
- **Measure the sign.** Measure sibling error correlation before and after KV-alignment distillation, and state the predicted direction first. That is the measurement form and it is the recommended one.
...
**The reasoning-not-knowledge caveat above is not this document's observation. It is established twice, with numbers.** Saunshi et al. (arXiv:2502.17416) give the iso-FLOP ablation: a looped 12x2 model scores **9.3% on closed-book QA against an iso-FLOP baseline's 11.2%**, and **34.3% on math word problems against 26.7%**, beating even a 24-layer non-looped reference at 29.3%. Looping **loses** on memorisation and wins on reasoning, measured. Ouro states it at 1.4B and 2.6B scale, verbatim: "this advantage stems not from increased knowledge capacity, but from superior knowledge manipulation capabilities." **Presenting the distinction as a finding of this architecture would repeat C-04 with two more prominent papers, and the distinction is used here strictly as a premise.**
...
**Built on the harness side, 2026-09-17.** The readout now exists as a typed head: `logos-harness/logos/arch/decision_head.py` reads a `noul` for the halt off the pooled state, trained by the Brier score against whether that hop's answer graded correct, with the threshold an ordinary tunable and the reading a reliability / resolution pair rather than a gate loss. `DECISION_TOWER.md` §2 and §3 carry the mapping from TypeSafe's Choice / Score / Noul to the harness questions and the first run's numbers. Since 2026-09-19 the exit is a `DEC` readout in the four-mechanics vocabulary of `../logos.tex` §mechanics; `DECISION_TOWER.md` §4 gives the mechanics column for this paper's own detectors.
...
**The defect L-01's method finds is different from X-05's, and it is upstream of it.** §11.1 presented one comparison as though it were the comparison. It is one case out of three, and the three have different signs. The paper now says so. It briefly went one step further than this document did and stated that the **decision rule inverts** under demand-proportional sizing, and it has since dropped that, because the inversion rested on case 3 recovering the throughput and case 3 does not recover it. The paper's current sentence, which is the register of record: *on throughput alone the ensemble never beats a fungible fleet of equal cost, and its serving-side case is that a skewed, stable, pre-known traffic mix lets it spend the same hardware on more parameters per unit of demand.* That is what §5.3 and §5.4 below already say: case 3 buys **parity**, and the strict win, if there is one, comes off the size axis in §5.4 and not off the domain axis.

**Two instantiations, and they differ in two ways rather than one.** `logos.tex` §11.1 computes its three cases on **five** domains at shares `(0.60, 0.20, 0.10, 0.05, 0.05)`; §5.3 below computes them on **four** domains at `(0.55, 0.20, 0.15, 0.10)`, because §2.5's criterion returns four. The arithmetic differs accordingly: the paper's case 2 loses a factor of 3 and two thirds of a 320-accelerator fleet to idleness, and §5.3's case 2 loses a factor of 2.20 and 54.5 percent of the fleet. **They are the same argument evaluated at different assumed skews on different domain counts, and neither skew is measured** — but they are *not* the same argument evaluated on the same comparator, and an earlier version of this paragraph said "both are the same argument" without flagging that. §5.3's comparator is a **fungible fleet of a generalist model** of the same per-instance size; §11.1's stated baseline was five replicas of one 2.8T **tower**, which covers one domain and is not a competitor to an ensemble at all (§5.3, and see the reconciliation below). **The paper has since been corrected to say "generalist" at that line**, so the two comparators now agree and only the skew and the domain count differ. Quote the paper's figures when citing the paper. Quote §5.3's when reasoning about the four-domain partition the criterion actually returns. Do not interleave them.

**One further correction the paper has since adopted, recorded here because §5.3 reaches it independently.** `logos.tex` §11.1's case 3 previously claimed that demand-proportional sizing recovers case 2's multiplexing loss. It does not, under the paper's own fixed-sparsity convention: pool `i` has `320 p_i` accelerators serving a tower that activates `s * 1.4e13 * p_i` parameters, so its token rate is independent of `i`, every pool serves at the same rate, and the fleet still saturates at `C/3`. §5.3 below reaches only **parity** for its own case 3 and states its capacity proportionality as an assumption rather than as a derivation, which was the right disclosure and is now the paper's position too.
...
- The **data-wall** argument, which X-04 left surviving only when the residency-bound corpus fraction `f` is below about 0.02 against the central token-supply estimate, that is only when almost the entire corpus is shared core. **`f` is now bounded above, by `f ≤ 1-g`.** That bound runs in the direction the data wall wants and says nothing about the direction the diversity claim needs, so it narrows the argument rather than settling it; the previous sentence here, that the bound "makes the argument unwinnable against the diversity claim", overstated it and is dropped per §1.5. The earlier statement that "`f` is unbounded anywhere in the paper" is also superseded; it was true of the version audited at v0.2 and is false of `logos.tex` v0.3.
...
and `REVIEW_ROUND2.md` A-02 found four defects, **all four of which the paper now discloses as (E1) to (E4)**. The disclosure does not fix the metric; it documents that the metric requires four separate corrections before it can be read. A metric that needs four caveats to be interpretable is not the metric to hang measurement F10 on.
...
- **A-02's own recommended fix is superseded.** A-02 recommended defining eta only above a stated denominator floor `δ` and keeping it. The paper went further and dropped the metric, and `GAPS.md` §4a records that **the paper is the register of record** on this. Anyone working from A-02's fix text is working from a superseded recommendation.
...
- **Sanity replicate:** a 350M base, matching `F9_MEASUREMENT_PLAN.md`'s stand-in scale, run as a cheap check that the effect is not an artifact of one base. Reported separately, never pooled.
- **`N = 8` adapters.**
...
Eight corpora chosen to be domain-disjoint. **Disjointness is measured before any training step**, applying `logos.tex` **§3.5**'s P1 criterion at small scale, which is exactly what F11 asks for and has never been run:
...
- a stated **maximum admissible pairwise overlap**, above which the corpus pair is rejected and replaced before training begins.
...
| **A5** | A2's members, calibrated-confidence weighting, ungated, on the same battery | Limb (b) of F13. Shares the battery with `F9_MEASUREMENT_PLAN.md`'s K5 arm so the two documents' results are comparable |
...
**A correction to this document's design, and the correction improves it.** An earlier draft asserted that testing anything below L5 needs separate pretraining runs and that those are out of reach on the owned card. **That is false at nano scale.** `F9_MEASUREMENT_PLAN.md` §8.1 costs a **125M pretrain at `T = 1.0e9` tokens at 9.579 GPU-hours** on the owned RTX 3090, at the corrected 29k tokens/s throughput. Full pretrains are therefore affordable, and the whole adapter-only framing of §7.3 understated what the card can do.
...
4. **One seed per arm.** The table above buys 17 runs at one seed. A second seed doubles it to 459.8 GPU-h. `F9_MEASUREMENT_PLAN.md` §9.4's rule applies verbatim: **a non-significant result at low seed count is reported UNDERPOWERED, not negative.**

### 7.4 Endpoints and refutation conditions

**Primary endpoint: error decorrelation.** `rho_bar`, the mean pairwise phi coefficient between members' per-item correctness vectors on a held-out battery. Compare `rho_bar(A2)` against `rho_bar(A3)` with a stated margin and a permutation test over member pairs.
...
**The pretrain replicate, priced separately and on a firmer basis.** §7.3.1's runs are costed against `F9_MEASUREMENT_PLAN.md` §8.1's **corrected and derived** 125M throughput of about 29k tokens/s, not against this subsection's assumed `r_train`, so these figures do not inherit the assumption above:
...
**Comparator:** `F9_MEASUREMENT_PLAN.md` §8.1 budgets **1,536.5** GPU-hours for the observation-bound experiment at its 1B-class proposer planning instantiation, band 1,376.8 to 3,773.3 (this line previously quoted the superseded 1,402.6). The adapter experiment is roughly one eighth of that and the pretrain replicate plus `g`-sweep is roughly one quarter; all three test different claims and none substitutes for another. `r_train` and the evaluation line are the two remaining assumptions in the adapter budget; both are measurable in a single short profiling run before committing the budget, and that profiling run should happen first. **The pretrain lines do not need it**, because F9's throughput correction already paid for that derivation on the same card at the same model size.
...
`logos.tex` §15 states F2's decisive observation as "composed ensemble scoring below a matched jointly post-trained baseline at any scale from 7B upward". `REVIEW_ROUND2.md` §7 observes that this is **already satisfied by the paper's own cited source at zero GPU cost**: Branch-Adapt-Route reports 49.1 composed against a jointly post-trained baseline of 50.5 with mid-training, and 49.1 < 50.5. §3.2 of the paper even concedes "Branch-Adapt-Route matches a retraining baseline; it does not beat one" without noticing that this satisfies its own measurement as written.
...
### 8.7 The extrapolation shape and what each reading means

The shape is stated before any point exists, so the readings below are not chosen to fit whatever comes back.
...
**Claim no novelty for the mechanism.** Image-as-tokens into a single autoregressive stream is **Emu3** and **Chameleon**, both already cited in `logos.tex` §12.6's own description of the harness, and the codebook construction is the RQ-VAE lineage `LOGOS_HARNESS.md` already names (TIGER, MAGVIT-v2, and the residual-quantizer implementation it specifies). Presenting any of it as new would repeat C-04 for a third time in this repository.
...
So the trade is: **pay roughly 20x compute per unit of content, to unlock content that has no text form at all.** That is trading the non-binding resource for the binding one, which is the correct direction whenever both statements above hold, and it is correct **because** of Proposition 1 rather than in spite of the penalty. **If Proposition 1 fails the argument fails with it**, and measurement F1 is what would take both down together.
...
2. **Corpus overlap** under `logos.tex` §3.5's P1 criterion, which is measurement **F11**, one of the two measurements of fourteen needing **no accelerator at all**, and which §7.2 runs as a by-product of the diversity experiment.
...
**The capability thesis is not.** Whether four 2.8T domain parents with adapters outperform a 14T monolith, and whether Branch-Adapt-Route survives a 400x extrapolation from 7B with four experts to 2.8T with four towers, is measurement **F2** and `ARCHITECTURE_REVIEW.md` **F-04**, which that document states cannot be closed by argument and needs a training run. **No experiment at this budget settles it.** Nothing in this document reduces that bet, hedges it, or makes it smaller. It only renames one of its components correctly (§4.2) and forces the lineage commitment it always required (§1.4).

**And one thing this document previously claimed was settled by arithmetic is not settled at all.** The previous revision said "§1.5's trilemma is not waiting on a run" and that "no setting of `g` satisfies X-04, C-02 and F-04 at once". **Both are dropped (§1.5).** `f <= 1-g` is an upper bound, so `g >= 0.98` suffices without being necessary, and the verdict is a function of a supply estimate spanning a factor of ten, over which `g*` runs from unsatisfiable at any `g` to 0.357. §1.5 is **waiting on two measurements**: the token-supply figure (which nothing here measures and which §10 is the one route to moving) and corpus overlap, which is measurement **F11** and needs no accelerator. This document still adopts response 1, but as a judgement under an unmeasured parameter and on the two legs that survive, not as a corner arithmetic forced it into.
...
**Two dependencies outside this file. Both were open in the previous revision and both are now resolved upstream; the resolutions are recorded here rather than left stale.**

- **Stand-in tower identity: RESOLVED.** `LOGOS_HARNESS.md` §2.2 settles it against both prior readings. Proposers are **frozen open-weight models of distinct pretraining lineage**, no gradient step anywhere, and the generation ledger is re-derived on their inference cost in `F9_MEASUREMENT_PLAN.md` §8.1. The 137.9 GPU-hour figure computed at 350M stand-in throughput is superseded. **What is not resolved is the roster**, which sets the generation line over a factor of 16, so every figure this document quotes from that ledger inherits a band and not a number; §9.3's emulator accounting quotes `F9_MEASUREMENT_PLAN.md`'s planning instantiation and says so.
- **The proposal interface: RESOLVED, and it is now specified rather than merely named.** `LOGOS_HARNESS.md` §3.4 specifies the **observation card**, a structured text block emitted deterministically by `bootstrap/render_observation.py` from the same (frame, RAM-state) tuple Phase 0 dumps, with a stated field list, a stated exclusion list, and four parity checks against the code rendering. The RQ-VAE is off the proposal path entirely. §9.3's emulator trajectory counts are therefore no longer blocked. **What remains unbuilt is the code**: `logos-harness/` does not exist in this repository, so the interface is specified and not implemented, and that is a different status from the one this bullet used to record.
...
**Dependencies on documents this file does not own**, per §11: stand-in tower identity (`LOGOS_HARNESS.md` §7 against `F9_MEASUREMENT_PLAN.md` §8.1), and the missing structured-text proposal interface (`LOGOS_HARNESS.md` §5.3).
...
14. ~~§3.5 and §12.3: weaken "disjoint pretraining" to "common seed, divergent continued pretraining", and state the lineage level.~~ **Applied**, as §3.3 plus measurement F14.
...
17. ~~§9.2: the operative dispatch ratio is 60 serial round trips per token against 1 per query, not 960 against 1.~~ **Applied.** The paper's §9.2 now reads "Sixty serial round trips per token against one per query is the difference that decides serviceability, and nine hundred and sixty expert invocations per token against one is the difference that decides bandwidth", and records the superseded sentence. It also now states the locality premise the comparison rests on, and the round-trip crossover of about 1.7 ms that scopes it to wide-area networks.
18. ~~`logos.tex` §11.1 computes its three cost cases on five towers while §3.5's criterion returns four.~~ **Applied.** §11.1's case 2 now carries the four-tower figure (58 percent idle, a factor of 2.4) alongside the five-tower one and says the two must not be interleaved, and the conclusion carries both.
19. **New, and closed in the same pass. `logos.tex` §11.1's case 3 claimed to recover case 2's multiplexing loss and does not.** Under Proposition 1's fixed sparsity every pool's token rate is independent of its share, so the fleet still saturates at `C/3`. The paper now drops the recovery claim, states the sparsity convention it is conditional on, and takes the weaker conclusion that case 3 buys per-domain parameter allocation at throughput parity. §5.3's parity result and §5.4's size-axis win are unaffected and were the correct answer all along.
...
Every table in this document is arithmetic over stated assumptions. §1.5's trilemma table is arithmetic over the paper's `f ≤ 1-g` against a **survey** token-supply figure that nothing in this repository verifies. §3.2's correlation table is a Beta-Binomial model with a chosen `p` and `k`. §5.3's cases use an illustrative traffic skew that nothing measures and a capacity proportionality that nothing verifies. §5.4's ratios use assumed exit rates. §6.4's degeneracy probabilities reuse A-02's own simulated per-cell rate. §7.6's adapter budget rests on an assumed training throughput that a short profiling run would settle and which nobody has run; its **pretrain** lines rest on `F9_MEASUREMENT_PLAN.md`'s corrected and derived 125M throughput, which is the firmest cost basis in this file. §9.6 and §9.7 are conjecture with a measurement attached and no measurement performed. **§10.2's density figures are estimates and a real pilot may make them worse by more than a factor of two.**

**Two numbers in this document were wrong in the previous revision and are corrected here rather than quietly adjusted.** §9.3 priced emulator trajectories at 1e4 tokens against a **frozen** 310-token trace geometry, a 32.3x overcount, and the conclusion it supported (that a million emulator trajectories bootstrap approximately the 277M edge tier) does **not survive**: the real figure is 7.75e6 parameters and the edge tier is 1.55 orders away. §7.4 asserted that testing below L5 was out of reach on the owned card, which is false at 9.579 GPU-hours per 125M pretrain, and the design is better for the correction.

The one thing this document was confident about was negative and was §1.4: **there is no composition mechanism that functions at L0**, so the paper's simultaneous demand for independently pretrained towers and for Branch-Train-MiX-style merging is not a design, it is two designs. That one stands. **The previous revision claimed a second negative of the same kind, that `f ≤ 1-g` means no setting of `g` satisfies X-04, C-02 and F-04 at once. It is dropped** (§1.5): the bound runs one way only, the verdict moves across the supply interval, and what survives is X-04 at MEDIUM restated in `f`. §1.4 is an argument and is offered as such; §1.5 is a sensitivity analysis over a survey figure and is now labelled as one.
