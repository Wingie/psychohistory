...
**The conflict that survives, and its severity.** It is not about `g`. Stated in `f`: a corpus overlap high enough to meet the central supply estimate is a corpus overlap high enough to cost the diversity claim. `logos.tex:719` states exactly that without mentioning `g`, and **round 2 already recorded it as X-04 at MEDIUM**. §3.3's contribution is an upper bound on `f`, while the binding constraint on the diversity side is a *lower* bound on `f`, about which `f <= 1-g` says nothing at all. This section is graded at X-04's severity until a supply measurement exists.
...
**Two inputs, reported honestly, because one of them was suspected here and is sound and the other is sound and hurts.**
...
**Response 1 is a real loss and it should be written as one:** it removes the third motivation of `logos.tex` §3.1, which that section itself calls "the one we think matters most". Removing it on a judgement rather than on a proof is a worse position to be in than the previous revision of this section claimed, and that is the honest statement.
...
**Comparator:** `F9_PREREGISTRATION.md` §8.1 budgets **1,536.5** GPU-hours for the observation-bound experiment at its 1B-class proposer planning instantiation, band 1,376.8 to 3,773.3 (this line previously quoted the superseded 1,402.6). The adapter experiment is roughly one eighth of that and the pretrain replicate plus `g`-sweep is roughly one quarter; all three test different claims and none substitutes for another. `r_train` and the evaluation line are the two remaining assumptions in the adapter budget; both are measurable in a single short profiling run before committing the budget, and that profiling run should happen first. **The pretrain lines do not need it**, because F9's throughput correction already paid for that derivation on the same card at the same model size.
...
- **Stand-in tower identity: RESOLVED.** `LOGOS_HARNESS.md` §2.2 settles it against both prior readings. Proposers are **frozen open-weight models of distinct pretraining lineage**, no gradient step anywhere, and the generation ledger is re-derived on their inference cost in `F9_PREREGISTRATION.md` §8.1. The 137.9 GPU-hour figure computed at 350M stand-in throughput is superseded. **What is not resolved is the roster**, which sets the generation line over a factor of 16, so every figure this document quotes from that ledger inherits a band and not a number; §9.3's emulator accounting quotes `F9_PREREGISTRATION.md`'s planning instantiation and says so.
