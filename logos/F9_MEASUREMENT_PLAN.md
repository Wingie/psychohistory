# Measurement plan: F9, the observation bound (`logos-harness`)

**Checklist item:** F9 (`logos.tex` §15 measurement table, row F9; `logos/GAPS.md` §2 Tier 0, P0). Also carries **F5** (by-product, §6) and **F13 limb (b)** (costed in §8.2), both of which `logos.tex` §15 scopes as arms or by-products of F9.

**Line references.** This document cites `logos.tex` and `logos/LOGOS_HARNESS.md` by SECTION, not by line number. Both files are under active revision and line numbers in earlier drafts of this document had already gone stale by the time the v0.2 corrections landed.
**Status of this document:** PLANNING. It is not a claim of results. It fixes every quantity that `logos/LOGOS_HARNESS.md` leaves free: sample size, seeds, endpoints, effect sizes, test statistics, multiplicity correction, stopping rule, and the gate, yield and round parameters that silently determine each arm's training corpus.
**Date drafted:** 2026-07-25.
**Source of record:** `logos.tex` §12 (`sec:observation`), its ordering claim, the F9 and F13 measurement rows in `logos.tex` §15, and the phase and gate table at `logos/LOGOS_HARNESS.md` §7.

**Revision note, proposer pass, 2026-07-25.** This version repairs a design defect that made F9 unrunnable as specified, and it is the largest change to the budget since the throughput correction. §8.1 of the previous revision generated proposals from "two 350M-class stand-in towers"; `LOGOS_HARNESS.md` §7 Phase 4 generated them from "at least two distinct open models". **Neither works**, and `LADDER_ARCHITECTURE.md` §10 had already recorded the conflict as unresolved. The repair is `LOGOS_HARNESS.md` §2.2: the observation gets **two renderings**, a proposer rendering read by frozen open-weight models of distinct lineage and a code rendering read by the learner. What moves here: §2.1 (the proposers, new), §3 (the battery carries both renderings), §4 (S4 restated over the outcome space, S5 added), §5.3 (the yield decision, new), §8.1 (the generation ledger re-derived), §8.2 (limb (b) re-costed), §8.4 (F13 limb (a) and F14 costed for the first time, new), §8.3 (the observation-loss ablation costed rather than left unbuildable), §9.3 (a sixth voiding condition), §11 (the roster and both renderings enter the calibration record) and §12.

**Revision note, 2026-07-25.** This version reconciles against the corrections that landed in `LOGOS_HARNESS.md` after the first draft was written: the superseded throughput figures (§5.4 of that file, reflected in the banner above and §8), the resolved observation tokenizer at 90 tokens per frame rather than 270 (§3.2 there, reflected in §5 here), the inverted model sizing that makes 125M the powered screen (§5.1 there, §8.1 here), the ungated-scoring mandate from finding X-12 (§2.1 there, §2 here), and measurement F13 limb (b), which `logos.tex` §15 scopes as an arm of F9 and which the previous budget did not pay for (§8.2 here). Every number touched was recomputed rather than scaled.

Values marked **[FIXED]** are the settings this plan runs at. Values marked **[CALIBRATED]** are set by a rule stated here and executed on a calibration pass BEFORE any experimental arm is scored; the rule, not the number, is what is fixed here.
...
1. **The F9 total is no longer a single number.** Generation, the `tau_JS` pool, S4, S5 and F13 limb (b) all run on frozen open-weight proposers, and their cost is linear in the proposers' parameter count. At 0.5B-class proposers the F9 total is **1,376.8 GPU-h**, at 1B-class **1,536.5**, at 8B-class **3,773.3** (§8.1). **The roster is not fixed in this document**, so the F9 rung is a band with a named planning instantiation, not a total. What settles it: fixing `proposers/roster.yaml` and measuring quantized proposer inference in the day-one probe.
2. **Generation is now the second largest line and it is bigger than it was.** It moves from 137.9 to 281.3 GPU-h at the planning instantiation (it stood at 393.9 until Study 2's forbidden per-seed corpus multiplier was removed, §8.1). The old figure was cheap because it was priced against proposers that would have had to be trained first, out of a budget that never paid for them, and that would have been the homogeneous pair finding C-02 says returns a martingale. **The old number bought an instrument that does not exist; the new one buys an instrument that does.**
...
**One thing this pass buys back.** The same proposer inventory is the instrument measurement **F13 limb (a)** and measurement **F14** require, so both acquire a derived cost for the first time (§8.4, **17.4 GPU-h**). Three documents, this one included, previously declined to assert it.
...
| Old figure in this document, planned off the superseded throughput | 718 | $145 to $180 | ~EUR 76 |
| Powered design, corrected throughput, superseded 350M stand-in proposers | 1,402.6 | $281 to $351 rented | 491 kWh, ~EUR 147 |
| Same, before Study 2's per-seed corpus multiplier and duplicated A0 arm were removed | 1,683.6 | $337 to $421 rented | 589 kWh, ~EUR 177 |
| **Powered design, frozen open-weight proposers, 1B-class planning instantiation** | **1,536.5** | **$307 to $384 rented** | **538 kWh, ~EUR 161** |
| Same at 0.5B-class proposers / at 8B-class | 1,376.8 / 3,773.3 | $275 to $344 / $755 to $943 | 482 / 1,321 kWh |
| 1B-class, if the day-one probe fires the frozen "reduce T" rule (§8.3) | ~1,298 | $260 to $324 | 454 kWh, ~EUR 136 |

Breakdown in §8.1. At the planning instantiation the powered programme is `python3: 1536.5/96 = 16.0` to `1536.5/72 = 21.3` times the current ledger line, and it is under three hundred and fifty dollars rented or about EUR 160 of electricity on a card that is already owned. **The underpowered version is therefore not a budget constraint. It is an unforced error, and this document exists to remove it before the first run rather than to discover it afterwards.**

`GAPS.md` §2 and §5 must be corrected from ~718 to **~1,540 GPU-h at the 1B-class planning instantiation, stated as a roster-dependent band**, and `GAPS.md` §5's "cheap decisive negative" must be qualified: at n <= 2 no negative is licensed at all. `GAPS.md` also carries F13 limb (b) at 12.7 GPU-h and limb (a) as unpriced; both are superseded by §8.2 and §8.4 here.
...
**Reason 2, scoring [NEW, finding X-12].** `LOGOS_HARNESS.md` §2.1 now mandates an ungated scoring arm wherever the harness produces a number that will be compared against an external baseline. The gate conditions the retained sample on predictor disagreement, disagreement correlates with item difficulty, and the bias direction is neither known in advance nor estimable from the subsample. **Any statistic computed on gated output is a statistic on a difficulty-biased subsample.** Brier scores, skill scores, hold rates against persistence or climatology or a market or a superforecaster panel are population statistics over a full declared question set, so feeding them a disagreement-conditioned subsample does not make them harder or easier, it makes them uninterpretable. The rule is: **gate for what you train on, do not gate for what you report.** A4 is the arm every external-baseline comparison runs in, including the Substrate-B validity probe of §12 if that probe is ever scored against an external baseline.
...
| §8.1 budget | Study 1 is 5 arms x 8 seeds = **40 runs**, and A4 alone is 8 runs = 8 x 9.579 = **76.6 GPU-h** at the corrected 125M throughput (the old figure of 33 GPU-h was planned off the superseded throughput) |
...
**Calibrated, not fixed here: which models.** The roster is fixed in `logos-harness/proposers/roster.yaml` with model ids, revisions, quantization and a lineage attestation per slot, recorded in §11 **before** the `tau_JS` calibration pass and therefore before any arm. It is not named here because naming it here would assert an availability and licence check this document has not run, and because the choice is a cost decision the day-one probe informs (§8.1).
...
### 3.2 What the primary endpoint can and cannot do
...
- **S3 mean admitted yield per arm** (`yield(tau) = -log P_M(o_observed | context, action)`, `LOGOS_HARNESS.md` §2), reported against the **identity prediction** `0.5*(H(P1)+H(P2)) + JS` on the same items. **This is a reported diagnostic and no longer a VOID condition**, because the version that was one could not fail: gating on JS selects higher expected yield by an exact identity in Jensen-Shannon divergence, so "mean admitted yield in A3 exceeds A1" holds for any two proposers including two random ones. Parity with the identity is the null; a **shortfall** means the environment is less surprising than the ensemble's own disagreement predicts, and that is the direction carrying information. See §9.3.
- **S4 proposer diversity.** Mean pairwise Jensen-Shannon divergence between the proposer models' distributions **over the outcome space `O`, in bits** (§2.1), on a fixed 5,000-item probe set in the proposer rendering, measured and recorded **before any training arm runs**. **If mean pairwise JS < 0.15 the experiment is VOID**: a null on the gate arms would otherwise be uninterpretable between "the gate does nothing" and "the proposers were not diverse".
...
- **S5 proposer competence [NEW, and repaired in the same pass it was written].** Each proposer's own accuracy on the **control** condition of the §3 battery, in the proposer rendering, measured on the same pass as S4 and before any training arm runs.

  **The gate as first written passed a uniformly random proposer half the time, and it is replaced.** "Strictly above chance" on 10,000 items at chance 0.25 has standard deviation 43.3, so a random proposer needs 2,501 correct and passes with probability **0.4954**; two random proposers both pass with probability 0.2454, which is exactly the failure mode the next paragraph says S5 exists to prevent. **Replacement:** the gate is a one-sided **exact-binomial lower confidence bound**, `LCB99 > 0.35`, per proposer, **Bonferroni-corrected across roster members**, on 10,000 items. Below that, the experiment is VOID.

  **Three further repairs, because the first version of the gate was wrong in three separable ways.** (i) **The battery is disjoint.** S5 previously scored proposers on the same control items the learner's `p_control` is computed on, and since proposer text is loss-bearing (`LOGOS_HARNESS.md` §5.2), selecting proposers for control competence inflates the learner's `p_control`, which is the **subtrahend** of the primary endpoint `g = p_heldout - p_control`, biasing `g` toward zero, that is toward the equivalence verdict. That is precisely the trade §10 refuses when it declines to widen the K3 margin, and S5 was performing it on the endpoint instead of on the margin. S5 now runs on a **separate gate battery of 10,000 control items**, same construction, disjoint items, never used to score the learner, built by `eval/battery_build.py --gate` and recorded separately in §11. (ii) **The roster is fixed before S5 runs and the attempt count is recorded.** With an unfrozen roster, VOID is a re-roll rather than a terminus, and three candidate pairs give `P(at least one passes by luck) = 0.5703`; nothing previously logged the number of attempts. (iii) **"The control condition" means §3's text-supported control set**, not the F13 persona control of `LOGOS_HARNESS.md` §1.1, under which "strictly above chance" has no defined chance level at all. Each proposer's **held-out** accuracy is reported alongside its control accuracy and the two must match within a tolerance stated in the §11 record, since otherwise roster choice alone moves the headline endpoint.
...
### 5.1 Sequence, packing and observation-fraction arithmetic [FIXED, recomputed against the resolved tokenizer]
...
python3: per trajectory token              1/0.25            =  4.0x
         ...
         (under the superseded flatten geometry it would have been 670/(0.25*130) = 20.6x)
```

**Neither the 4.0x nor the 9.5x figure may be quoted as if it were the headline.** The headline is per-total-token. The collapse decision roughly doubles the differentiating signal per training token at fixed phi (10.5% loss-bearing against 4.9%), which is a real gain in effective power at zero cost, and it is the reason the 9.5x rescaling is smaller than it would have been. It does not change the primary endpoint, the fixed effect size, or any threshold in §8.
...
### 5.2 Secondary matching MP2 [FIXED]
...
### 5.3 Yield is scored under the PROPOSER ensemble, not the learner [FIXED]

`logos.tex` §12 and `LOGOS_HARNESS.md` §2 define yield as the surprisal of the observed outcome "under the ensemble's own prediction before it acted". Under the two-path split of §2.1 that sentence has two readings and **neither document said which**, so the corpus each arm trains on was undetermined. Fixed here:
...
**S3's manipulation check is read against this same object** (§4), and it is now read against the identity prediction rather than against A1, for the reason §9.3 gives: `H(P_M) = 0.5*(H(P1)+H(P2)) + JS(P1,P2)` exactly, so "A3's mean admitted yield exceeds A1's" is an algebraic consequence of the gate rather than a check on it.

---

## 6. The collapse monitor, specified [FIXED rule, CALIBRATED thresholds]

`LOGOS_HARNESS.md` §6 names `collapse_monitor.py` and never specifies it. This section is the specification.

**Probe set P:** 2,000 sequences of 2,048 tokens of natural text from the same source as the training text, disjoint from all training data, fixed at Phase 3, recorded in §11. All statistics teacher-forced, so decoding temperature cannot confound anything.
...
**Study 2's round budget, corrected [FIXED].** `LOGOS_HARNESS.md` §2 step 7 says "retrain incrementally, repeat", and §7 of this document sets R = 5 for Study 2. An earlier draft of §8.1 costed each round as a **full 1.0e9-token training run**, which is wrong on the design's own terms in two ways, and both would have invalidated Study 2 rather than merely overcharging for it:
...
**Fixed:** Study 2 holds total tokens per seed at 1.0e9, identical to Study 1, and partitions that budget into R = 5 sequential segments of 2.0e8 tokens with the corpus accumulating (never rotating) between segments. Checkpoints for the fire rule remain the fixed grid {25, 50, 75, 100}% of steps, which now straddles round boundaries at 1.25, 2.5, 3.75 and 5 rounds. A0 trains the same 1.0e9 tokens at R = 1, so the matched-step baseline is defined everywhere. Study 2 is therefore 3 arms x 3 seeds = **9 training runs, not 33**, of which **6 are novel** because A0 at `R = 1` is the same run Study 1 already performs and is reused at matched seed index, so the cost falls from a mis-derived 316.1 GPU-h to 57.5 (§8.1). **The trajectory-generation cost falls too, and an earlier version of this sentence said it did not.** It said "a multi-round corpus is model-dependent and every seed must generate its own", which contradicts §5.3's fixed rule: the proposers are frozen, `yield_score.py` loads no training checkpoint, and nothing in the admitted corpus can depend on the training seed in any round. One corpus per arm, shared across the three seeds, exactly as Study 1 does (§8.1).

---

## 7. Analysis plan and multiplicity [FIXED]
...
## 8. Sample size, power and cost [FIXED]
...
**Power table [FIXED, five contrasts].** Superiority MDE = sigma x sqrt(2/n) x (z_{1-0.05/5} + z_{0.80}) = sigma x sqrt(2/n) x 3.1680. TOST margin at 80% power = sigma x sqrt(2 x (z_{0.95} + z_{0.80})^2 / n) = sigma x sqrt(12.365/n).
...
**n = 8 is still the knee, and the fifth arm costs 0.099 sigma of MDE.** Correcting the multiplicity constant from three contrasts to five moves the n = 8 MDE from 1.485 to 1.584 sigma, which at the planning sigma is 0.0742 to 0.0792 accuracy points. That is 53% of delta = 0.15, so the design retains above 80% power on the effect it exists to detect. n = 16 buys 1.584 to 1.120 sigma for double the training cost; n = 5 degrades to 2.004 and n = 3 to 2.587. n = 8 is also the smallest n at which the equivalence margin, 1.243 sigma or about 6.2 accuracy points, is tight enough that a declared negative is worth reporting.
...
### 8.1 Studies and budget [FIXED, re-derived against the corrected throughput]
...
**Step 2: the trajectory-generation ledger, re-derived against the frozen open-weight proposers.** The previous revision priced this line at 137.9 GPU-h against **two 350M-class stand-in towers that nothing in the budget trained**, and §2.1 has replaced that instrument. What is unchanged: proposal volume, the 960 tokens processed per proposal (2 proposers x about 400 tokens of card-plus-protocol prefill plus about 80 generated tokens), the 4x over-generation in the gated arms, and the 310-token trace of §5.1. **What changes is the throughput the tokens are priced at, because the proposers are no longer 350M.**

**The throughput model, calibrated so that it reproduces the ledger it replaces.** The 9.1k tok/s at 350M and 2.526 GFLOP/token imply a sustained rate, and forward-only inference is one third of forward-plus-backward:
...
python3: traces for U=1.25e8 (125M) = 1.25e8/310 = 403,226 ; for U=2.5e8 (350M) = 806,452
         ...
         Study 2  A1 ungated + A3 gated, ONE corpus per arm shared across the 3 seeds
                                              : 403226 + 403226/0.25    =  2,016,130
         Study 3  A1 ungated + A3 gated at 350M  : 806452 + 806452/0.25   = 4,032,258
         TOTAL 10,080,646 proposals x 960 tokens = 9.67742e9 tokens processed
```

**Study 2's corpus is NOT per-seed, and an earlier revision charged a 3x multiplier that §5.3's fixed rule forbids [CORRECTED].** That revision read "3 seeds x (A1 ungated + A3 gated), corpora model-dependent across R=5 rounds so EACH SEED generates its own : 3*(403226+403226/0.25) = 6,048,387", and §6's round-budget note said "the trajectory-generation cost does not fall, because a multi-round corpus is model-dependent and every seed must generate its own". **Both are wrong on this document's own terms.** §5.3 freezes `P_M` as the proposer ensemble's pre-action distribution, "computed once, at generation time, by `bootstrap/yield_score.py`, which **loads no training checkpoint**"; §5.3 reason 2 says "the learner never acts; it reads traces afterwards"; and `LOGOS_HARNESS.md` §2.2 puts "gradient: none, anywhere, ever" on the proposer path. **If no learner state enters the proposal, the gate, the action or the admission path, then nothing in the admitted corpus can depend on the training seed, in round 1 or in round 5.** §5.3's own reason 2 uses exactly that argument to avoid an 8x multiplier in Study 1; applying a 3x multiplier of the same kind to Study 2 contradicted it inside one document. Three rescues were tried and none holds: "model-dependent" meaning proposer-dependent still gives one corpus for all three seeds; using the trained learner as a proposer in later rounds is forbidden by §2.1; and the emulator state cannot diverge per seed, because the actions come from frozen proposers under a seeded selection stream that is a property of the corpus and not of the training run. **Overcharge removed: 112.5 GPU-h at the planning instantiation.** The same argument is why `LOGOS_HARNESS.md` §2 step 7's "disagreement shrinks over rounds" is not instantiated here: frozen proposers cannot learn.

**And the answer is not a single number.**

```
python3: generation GPU-h = 9.67742e9 / throughput(N) / 3600
         2 x 0.5B :   140.7  2 x 1.0B :   281.3  2 x 1.5B :   422.0
         2 x 3.0B :   844.0  2 x 7.0B : 1,969.4  2 x 8.0B : 2,251.6
         (superseded, at the superseded 14,112,903-proposal volume: 196.9 / 393.9 / 590.8 /
          1,181.6 / 2,757.1 / 3,151.0)
```
...
**Planning instantiation [NOT fixed]: two proposers of about 1B parameters, distinct lineage, 4-bit.** Chosen because it holds generation to under a quarter of the four training lines (`python3: 281.3/1188.1 = 0.24`) at a size where distinct-lineage open-weight models are plentiful, not because any roster has been checked. At 3B the generation line alone is 844.0 and is most of the training programme; at 8B it is nearly twice it. **This is a planning figure and it is not a commitment to any model.** At that instantiation:

```
python3: Study 1: 4032258*960/9555/3600 = 112.5 GPU-h
         Study 2: 2016130*960/9555/3600 =  56.3 GPU-h   (was 168.8 under the removed 3x)
         Study 3: 4032258*960/9555/3600 = 112.5 GPU-h
         TOTAL GENERATION = 281.3 GPU-h        (was 393.9; and 137.9 against untrained stand-ins)
```

**One saving that exists and is not taken.** The roughly 150-token protocol prompt is identical across every proposal, so a prefix cache pays it once instead of 10.1 million times, cutting the per-proposal token count from 960 to about 660 and the line by about 31%. It is not taken because it requires the prompt to be a strict prefix in every proposer's template and this document has not verified that per roster. It is recorded so the lever is visible rather than discovered late. **This rule binds §8.4 as well, and §8.4 previously broke it**; see the band there.
...
| **Study 2, collapse (S2)** | 125M / 1.0e9 | A1, A3 novel; **A0 reused from Study 1 at matched seed index** | 3 | 5 within the same 1.0e9 (A0: 1) | 6 novel | **57.5** |
| **Study 3, confirmatory scale** | 350M / 2.0e9 | A0, A1, A3 | 3 | 1 | 9 | **549.5** |
| Outlier replacement reserve, 20% of training | | | | | ~12 | 198.0 |
| **Trajectory generation**, all arms, all studies, **at the 1B-class planning instantiation** | | | | | | **281.3** |
...
| **TOTAL at the planning instantiation** | | | | | | **1,536.5, call it ~1,540** |

```
python3: 383.1+57.5+549.5+198.0+281.3+1.81+15.0+12.0+36.3+2.0 = 1536.5
         tau_JS pool + S4 + S5 tokens = 50000*960 + 5000*2*480 + 10000*2*480 = 6.24e7
                                        6.24e7/9555/3600 = 1.81 GPU-h
         reserve = 0.20 * (383.1+57.5+549.5) = 0.20 * 990.1 = 198.0
```

**Study 2's A0 arm duplicated three Study-1 runs, and that is removed too.** §6 freezes "A0 trains the same 1.0e9 tokens at `R = 1`", so A0 has no trajectory data by construction and no round structure to vary; Study 1 already runs A0 at 125M, `T = 1.0e9`, `R = 1`, seeds 1001 to 1008. Study 2's A0 at seeds 1001 to 1003 is therefore **the same run**, and it is reused at matched seed index rather than re-run. Recomputed: 6 novel runs, `python3: 6*9.5785 = 57.47`, against the 86.21 this row used to carry, an overcharge of 28.7 plus 5.7 in the reserve. The `--rounds 5` passed to A0 in §13's reproduce block was the corroborating symptom and is corrected there.
...
|---|---:|---:|---:|---:|---|---|
| 2 x 0.5B-class | 140.7 | 0.91 | 18.1 | **1,376.8** | 482 kWh, EUR 145 | $275 to $344 |
| **2 x 1B-class [planning]** | **281.3** | **1.81** | **36.3** | **1,536.5** | 538 kWh, EUR 161 | $307 to $384 |
| 2 x 3B-class | 844.0 | 5.44 | 108.8 | **2,175.3** | 761 kWh, EUR 228 | $435 to $544 |
| 2 x 8B-class | 2,251.6 | 14.51 | 290.1 | **3,773.3** | 1,321 kWh, EUR 396 | $755 to $943 |

The seven training and probe lines (**1,217.1** GPU-h) do not move with the roster. **The band is the roster, not the statistics.** The previous version of this table read 1,467.6 / 1,683.6 / 2,547.4 / 4,707.2 on a fixed-line total of 1,251.6; both are superseded by the Study-2 corrections above and are recorded here so the drift is visible.
...
| Study 2 | 135, then 86.2 | 57.5 | throughput correction (+2.32x), the round-accounting correction of §6 (33 runs to 9), and now the removal of the duplicated A0 arm (9 runs to 6 novel), A0 being reused from Study 1 at matched seed index |
...
| Reserve | 101, then 203.8 | 198.0 | 20% of the corrected training cost |
| Generation | 75, then 137.9, then 393.9 | 281.3 | derived at 137.9 against 350M stand-ins that nothing trained; re-derived against frozen open-weight proposers at the 1B-class planning instantiation; and now with Study 2's forbidden per-seed corpus multiplier removed. **Roster-dependent, 140.7 to 2,251.6** |
...
python3: per proposer per seed, at proposer size N and throughput(N) from §8.1:
         ...
         at 2 x 350M (the superseded instrument): 0.104 + 0.313 + 0.244 = 0.661 ; x16 = 10.6 ; +20% = 12.7
...
**Design constraints, fixed so a positive is not over-read.**
...
**The protocol [FIXED as the costing basis, not as a design freeze].** Limb (a) asks whether debate between models of genuinely different pretraining lineage tracks the martingale as closely as debate between personas of one model. F14 asks the same question with a third condition, two continued-pretraining branches of one base. One instrument, three conditions, one pass:

- **M = 2,000 items**, drawn from the §3 battery in the proposer rendering with RNG seed 20260725, so the sample is reproducible. **Tense corrected:** an earlier revision said the items already exist. They do not: `logos-harness/` does not exist in this repository. The battery **is built before this limb runs**.
- **R = 3 debate rounds**, 2 agents per pair, each round contributing about 150 tokens of argument. With prefix caching each agent processes the transcript once: about 400 tokens of item and protocol plus 2 agents x 3 rounds x 150 = **1,300 tokens per agent per item**.
  ...
  **That is the same prefix-caching assumption §8.1 explicitly declines to bank, and quoting only the cached case here contradicted it.** §8.1 records a 31 percent generation saving as "one saving that exists and is not taken ... because it requires the prompt to be a strict prefix in every proposer's template and this document has not verified that per roster", and §8.4's roster (four models at 7 to 8B) is *less* checked than §8.1's, not more. The blocker is template compatibility, it is roster-side, and it does not weaken with scale. **Both cases are therefore quoted.** Without caching, each round re-prefills the grown transcript: `python3: (400+150) + (700+150) + (1000+150) = 2,550` tokens per agent per item, a factor of `2550/1300 = 1.96`, so the token total is `1.2242e8` and the figure is `python3: 1.2242e8/1194/3600 = 28.5` hours plus 20 percent slack = **34.2 GPU-h**. **17.4 is the cached case and 34.2 is the uncached case**, the headline figure is 2x sensitive to an assumption this document declares unverified, and the day-one probe of §8.3 verifies template prefix-compatibility per roster before either is committed.
...
2. **The only permitted early termination is technical**: a run diverges (loss NaN or inf) or trips the blind outlier rule of §7. It is replaced by a fresh seed from the declared seed list `[1001..1024]`, consumed in order.
3. **Voiding conditions**, checked before any endpoint is computed: (a) S4 proposer diversity < 0.15 over `O`; (b) **[REMOVED]**; (c) the leak scan finds any held-out term on any of the three surfaces of §3.1, or any control term occurring fewer than 1,000 times; (d) the Phase-1 RQ-VAE reconstruction gate of `LOGOS_HARNESS.md` §7 fails; (e) the Phase-2 trace-terminal-span assertion fails, that is, any trace ends on a loss-masked span, which would give the result frame exactly zero gradient and manufacture a false negative before any GPU ran; **(f) any proposer fails S5 at `LCB99 > 0.35` Bonferroni-corrected on the disjoint gate battery, or the observation-card parity gate of `LOGOS_HARNESS.md` §3.4 fails** — a field-list mismatch against the Phase-1 certified fields, an audit whose upper 95 percent binomial card-error bound exceeds 0.5 percent on the four gate fields or 2 percent overall, a card field the cross-validated code-recoverability probe cannot recover above its permutation floor, or a failure of the **anteriority assertion** (check 4: the card is not byte-identical when regenerated from the pre-action emulator state, or a card carries a text-box field with game-message content). A parity failure means the proposers and the learner were looking at different observations, under which every gate contrast is a comparison between two different experiments. Any of these gives **VOID, no directional conclusion**.

   **(b) is removed and the removal is the point.** It read: "S3 mean admitted yield in A3 not exceeding A1, scored under §5.3's proposer ensemble". That condition **cannot fail**. Jensen-Shannon divergence *is* the excess entropy of the mixture, `H(P_M) = 0.5*(H(P1)+H(P2)) + JS(P1,P2)` exactly, and expected yield on an item whose outcomes track `P_M` is `E[-ln P_M(o)] = H(P_M)`, so gating on `JS >= tau_JS` selects higher-expected-yield items **by identity**, one nat of yield per nat of divergence, for any two proposers including two uniformly random ones. On `LOGOS_HARNESS.md` §5.3's own printed vectors both sides come to 4.9420 bits. And if the comparator is A1, whose pseudo-outcome is `argmax P_M`, its yield is the surprisal of the mode, the minimum over `O`, so the comparison is won before it is run. A VOID condition that passes by construction certifies nothing, and separating yield in nats from JS in bits (§5.3) is what hid the identity from its own authors. **It is replaced by a reported diagnostic, not by another gate:** the gated set's realised mean yield is compared against the identity prediction `0.5*ΣH + JS` on the same items, parity is the null, and a **shortfall** is the informative signal, meaning the environment is less surprising than the ensemble's own disagreement predicts. `LOGOS_HARNESS.md` §7 carries the same replacement.
4. **Underpowered condition.** If realised sigma_hat makes the realised MDE exceed the declared **0.0792** (§8, five-contrast planning constant at n = 8), the run is reported **UNDERPOWERED**, the realised MDE and TOST margin are published, and no equivalence claim is made. The superseded figure of 0.074 was computed at a three-contrast constant and does not apply to a five-arm design.
...
## 10. What would kill the programme [FIXED]
...
**K5, the gain is in the protocol and not in the observation channel [NEW].** F13 limb (b) (§8.2) fires: calibrated-confidence weighting alone lifts ensemble accuracy on the held-out battery, with no environment adjudication of any kind, by a margin that survives exact McNemar at one-sided alpha = 0.05. Then the submartingale that breaks the debate martingale is protocol-internal, the observation bound is not what limits the loop, and `logos.tex` §12's central claim is located in the wrong place. **Steelman, recorded so a positive is not over-read:** Zhu et al. purchase calibrated confidence with external supervision, so limb (b) firing does not show that exogenous signal is unnecessary, only that it can enter through the calibrator instead of through the environment. K5 therefore refutes the *channel* claim and not the *exogenous signal* claim, and the write-up must say which. If limb (b) fires AND C1 also passes, both are reported; they are not mutually exclusive and neither cancels the other.

No result here is renegotiated after the numbers land. The rule above is the whole verdict.

---

## 11. Calibration record [FIXED]

**Post-calibration addendum**, appended after the calibration pass and before any experimental arm is scored: tau_JS at q = 0.25; s(M1), s(M2), s(M3) from the A0 calibration; **measured 3090 tokens/s at 125M and 350M, and the resulting GPU-hour total recomputed from the measurement rather than from the 71-TFLOPS ceiling**; whether the 60% rule of §8.3 fired and which contingent row it selected; the S4 proposer-diversity value; **the S5 per-proposer control accuracy and the per-proposer malformed-proposal rate**; **the measured proposer inference throughput and the generation total recomputed from it rather than from the §8.1 planning instantiation**; the leak-scan count and the per-control-term occurrence counts; the measured admission rate at tau_JS.

The configuration this runs against is the arms file (A0 to A4, five arms), the RQ-VAE config (90 positions, 3 levels, collapsed per position), the proposer roster (model ids, revisions, quantization, lineage attestation), the outcome-space file (`O_A`, `O_B`, the label tokens, the floor `eps`), the calibrated-confidence LoRA adapters for both proposers over 8 seeds (§8.2), the held-out vocabulary file, the bootstrap config, the evaluation battery in **both renderings** (§3), the observation-card parity report (field-list diff, 1,000-frame audit, code-recoverability probe, §3.4 there), the collapse probe set P, and the seed list `[1001..1024]`.
...
- **The throughput figures this plan is costed against are still arithmetic, not measurement.** The original 18k to 30k tok/s at 350M was an interpolation from 4090, A30 and L20 runs that exceeded the RTX 3090's dense BF16 ceiling; `LOGOS_HARNESS.md` §5.4 has replaced it with about 9.1k tok/s at 350M and about 29k at 125M, derived against 71 TFLOPS dense and an assumed 25% to 35% MFU. That is a defensible ceiling-based derivation and it is **not** a first-party 3090 log. The day-one probe of §8.3 is the resolution, the fixed response to a shortfall is to reduce T and never n, and until the probe runs the total in §8.1 carries a band from the MFU assumption alone. **That band was previously quoted as "roughly 1,200 to 1,800 GPU-h", which is the superseded 1,402.6 total rescaled by 30/25 and 30/35 rather than a band on any current total, and it failed to overlap `TIER0_3090_PLAN.md`'s "roughly 2,100 ... roughly 1,450" for the same quantity at either endpoint.** Derived once, here, and quoted from here everywhere: at 2.526 GFLOP per token against 71 TFLOPS dense, 25 percent MFU is `python3: 0.25*71e12/2.526e9 = 7,027` tok/s and 35 percent is 9,838, against the planning 9,100, so the total scales by 1.295 and 0.925 and the band on 1,536.5 is **1,421 to 1,990 GPU-h**. Separately, §0's quoted throughput band of 6.6k to 9.3k tok/s corresponds to MFU 23.5 to 33.1 percent rather than the 25 to 35 percent stated here; one pair is stated and the other is derived from it.
...
- **Any number this harness reports against an external baseline is scored in the ungated arm A4, never on gated output** (§2 Reason 2, finding X-12). This binds Substrate B in particular: a Brier score, skill score or hold rate against persistence, climatology, a market or a superforecaster panel is a population statistic over a full declared question set, and the disagreement gate selects exactly the difficulty-biased subsample on which such a comparison cannot be scored.
- Substrate B (the psychohistory validation pipeline as adjudicator) is **not** part of this plan's primary or secondary endpoints. A count of the rosters gives 83 rows, 68 distinct onset dates, 58 distinct real-world episodes and 35 carrying a committed endogenous/exogenous label, which caps a paired Substrate-B contrast at a minimum detectable effect of about 0.42 sigma before any disagreement gate discards anything. Substrate B is a directional sanity check that can refute and cannot confirm, and a null from it is not evidence of absence.
- **The proposals do not come from towers, and after the proposer pass they do not come from anything trained for this experiment either.** They come from frozen open-weight models of distinct lineage reading a text or image rendering of the observation (§2.1). That is a better instrument than the superseded 350M stand-ins, which nothing paid to train and which would have been the homogeneous pair the martingale result covers, and it is still not a Mixture-of-Towers. No claim here transfers to tower scale.
...
- **S4's 0.15 threshold is inherited, not re-derived against the outcome space it is now computed on** (§4). It is a VOID gate, so it cannot manufacture a result, and the realised distribution is published in the §11 record either way.
- **`logos.tex` §15 no longer disagrees with this document, and the previous version of this rail was itself the stale claim.** It said the paper "prices F13 limb (b) at 12.7 GPU-hours and says limb (a)'s cost is not derived" and that "the paper has not been edited". Verified: `grep -n "12.7" ../logos.tex` and `grep -n "not derived" ../logos.tex` both return no matches, and §15 carries **36.3** and **17.4**, repeated at four places including the conclusion. The edits landed and three documents in this directory went on recording them as owed. The rail is inverted rather than deleted, because the failure mode it records is real and was committed here.
...
py -3.12 logos-harness/eval/battery_build.py          # fixed battery, BOTH renderings
...
py -3.12 logos-harness/train/run_arm.py --arm {A1,A3} --model 125m --rounds 5 --seed {1001..1003}
                                                      # Study 2. A0 is NOT re-run: §6 freezes it at
                                                      # R = 1 and Study 1 already produced it at
                                                      # seeds 1001-1003. An earlier revision passed
                                                      # --rounds 5 to A0, contradicting §6 and
                                                      # double-charging three runs (§8.1)
...
py -3.12 logos-harness/analysis/analyze_f9.py         # evaluate the rule ONCE
```
