...
1. **P0** — If QB/CDB are going to be load-bearing, either (a) find or write the paper, or (b) treat the `logos/validation/L3_quantile_balancing/` implementation as the specification of record. Currently the architecture depends on two mechanisms whose only description is prose on a blog.
