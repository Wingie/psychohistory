...
1. **`p_outcome`, a categorical distribution over the pre-committed outcome space `O`** (§2.3), obtained by constrained decoding over single-token category labels and renormalised over the label set. Never by parsing free text.
...
3. **free-text prediction and reasoning**, which enters the trace (§5.3) and enters nothing else.
...
> (effectiveness bucket ∈ {no effect, not very, neutral, super}) × (damage bucket ∈ 5 quintiles of the defender's max HP) × (faint ∈ {yes, no}), so **|O_A| = 40**.
...
The proposer path of §2.2 needs a rendering an off-the-shelf model can read. There are two, and a run declares which it uses per proposer.
...
2. **Paired human-or-OCR audit** on a frozen sample of **1,000 held-out frames**: does each card field match what the rendered frame displays? Pass condition: **100% on the four Phase-1 gate fields** and **at least 99% overall**. Frames are the same held-out split the Phase-1 gate uses, so the two gates are read on one sample.
3. **Code-side recoverability probe.** For each card field, fit a linear probe from the 90 collapsed codes of the same frame to that field, on held-out frames. A field the probe cannot recover above a floor pre-committed with the probe is a field the learner's codes do not carry, and **it is dropped from the card** rather than kept, because the proposer must not condition on what the learner cannot see. The probes are linear over cached codes and add under 0.1 GPU-h, absorbed in the RQ-VAE line of `F9_PREREGISTRATION.md` §8.1.

Checks 1 and 2 need **Phase 0 only**. Check 3 needs Phase 1, so it runs when the RQ-VAE gate passes, and a card field it drops is a card change that re-runs check 2 on the same sample. Nothing here is a GPU cost of any size; the human audit is not GPU work and is not priced in GPU-hours.
...
| **4** | **Bootstrap loop, Substrate A.** Proposal by **≥2 frozen open-weight models of distinct pretraining lineage, reading the §3.4 observation card and never the codes** (§2.2), each returning a distribution over `O_A`; JS-divergence gate over `O_A` in bits; emulator adjudication; yield scored under the **proposer** ensemble (§2.3); accumulating admission. Arms A0 to A4 per `F9_PREREGISTRATION.md` §2, including **A4 ungated grounded**, without which the gate effect under grounding is unidentified | **Before any arm runs:** the proposer roster is frozen with lineage attested and digests lodged; the §3.4 parity checks pass; **S4** proposer diversity measured, mean pairwise JS >= 0.15 or the experiment is VOID; **S5** proposer competence, each proposer strictly above chance on the control condition or the experiment is VOID (`F9_PREREGISTRATION.md` §4); `tau_JS` calibrated to q = 0.25 on a 50,000-proposal pool. **Then:** admitted trajectories have **mean yield strictly above** the unfiltered control (if not, the run is VOID, not negative); disagreement shrinks over rounds in explored regions |
...
**The phase numbers are not the run order, and the two used to be inconsistent.** Phases are a dependency order for what is *built*; `TIER0_3090_PLAN.md` carries the order things are *run* in, and under §2.2 that order changes: the Phase-4 gates (S4, S5, `tau_JS`) and F13 limb (b) depend on **Phase 0 only**, because the proposers read the observation card and the card is built from the Phase-0 dump. Nothing on the proposal path waits for the Phase-1 RQ-VAE. Under the withdrawn stand-in reading every one of those gates needed the codes, and the run order that scheduled them before Phase 1 could not have executed.
...
| **Towers stand in for towers.** Frozen open models of 0.5B to 8B class are not 2.8T towers, and the disagreement structure may not transfer | Unavoidable at this budget. State it; do not claim validation at tower scale. Two things *are* measured before any arm runs: **S4**, mean pairwise JS >= 0.15 over `O`, or VOID; and **S5**, each proposer strictly above chance on the control condition, or VOID. S4 alone was not enough, because two incompetent proposers disagreeing at random satisfy it while turning the gate into a random sampler |
...
| **The generation line is now proposer-size-dominated, and the roster is not frozen** | Disclosed, not solved. Generation scales linearly in proposer parameters: 196.9 GPU-h at 0.5B against 3,151 at 8B (`F9_PREREGISTRATION.md` §8.1). **The F9 total is a band until the roster is frozen**, the run order puts the roster freeze and a proposer-inference measurement early, and no single total is asserted as if the roster were settled |
...
**One correction owed upstream, recorded rather than silently absorbed.** `../logos.tex` §15 prices F13 limb (b) at **12.7 GPU-hours** and says limb (a)'s cost is not derived. Both statements were true against the withdrawn 350M stand-ins and neither survives §2.2: limb (b) re-costs to **36.3 GPU-h** at the §8.1 planning instantiation and moves with the roster, and limb (a) with F14 is now derived at **17.4 GPU-h**. The paper is the register of record and this document does not overwrite it; the edits are owed there, and until they land the two disagree and this note is where that is said.
