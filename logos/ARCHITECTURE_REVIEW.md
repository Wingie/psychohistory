...
| F-04 | BAR extrapolated 400× with no hedge | **HIGH**, unresolvable by argument | **Named, still unhedged**, and now entangled. §**3.4** (`sec:extrapolation`, renumbered from §3.3 when §3.3 became the lineage section), falsifier F2. The central bet. §3.3 adds what F-04 did not see: BAR is also what forces lineage sharing to `λ ≥ 1`, so F-04, round-2 **X-04** and round-2 **C-02** are three views of one parameter (`GAPS.md` §4a). Dropping BAR to rescue the diversity claim removes the method F-04 is the extrapolation of |
...
| F-12 | No cost model, no evaluation method for a composed ensemble | **MEDIUM** | **Solved for single-tower serving; the §12 loop workload is unpriced** (round-2 X-06). §11.1 now gives a bracket of 106 to 320 accelerators and three traffic cases rather than one, and §11.2 **retires η**: the metric is **AIQ** (RouterBench) as the primary scalar with **APGR** and **CPT(50%)** (RouteLLM) as the secondary pair, all adopted from published work and claimed as no contribution. The η this row used to name is superseded; see `GAPS.md` §3 |
...
**A sixth step has since been added by the paper and this review did not see it coming.** `logos.tex` §3.3 names **lineage sharing** as an explicit design parameter and shows that steps 3 and 5 above are competing for the same knob. Step 3's peak-unique-corpus saving survives residency constraints only when the shared core is above about 98 percent of each tower's corpus; the informative-difference argument, which this review's F-04 body and round-2's C-02 both bear on, needs the towers *not* to share a corpus. Branch-Adapt-Route settles it at `λ = 1` because the architecture cannot drop the method, so the corrected spine ends:

6. **Every tower is a divergent continued-pretraining branch of one common seed, not an independent model**, and the paper's three tower motivations (update economics, alignment isolation, informative difference) are priced against that weaker object rather than the stronger one the abstract used to imply. Whether the weaker object still carries the diversity conjecture is **falsifier F14**, it runs on one consumer accelerator, and it has not been run.
