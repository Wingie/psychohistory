...
> ## What this document's method could not catch
>
> This document checks the architecture's arithmetic and its internal logic. Four classes of defect sit outside that, and all four are present here.
>
> 1. **A citation that resolves, is real, is quoted accurately, and supports the opposite of the inference drawn from it.** Both debate papers are cited in `logos.tex` for a diversity claim that both explicitly disclaim, and the claim is printed in the abstract (finding C-02). Verifying that a citation exists and is quoted correctly cannot detect this. It collapses the only architectural item in the paper's list of three original contributions, and this document passed over it entirely.
> 2. **A claim with no citation at all.** `logos.tex:94`, the token-supply premise the entire data wall rests on, carries no citation and no number; `logos.tex:172`, one of two enumerated motivations for the tower split, asserts a measurement with no source. Neither is listed here or in `BIBLIOGRAPHY_REVIEW.md`.
> 3. **A derivation this document reproduced rather than checked.** F-01 below computes `N_act ≈ 5e10` from the expert fraction and treats it as ground truth. Finding A-01 shows that method understates by 1.54x on Kimi K2 and 1.76x on DeepSeek-V3, the only two shipped models where ground truth exists, because it ignores always-active attention, embeddings and shared experts. F-01's diagnosis is right and its input is wrong. Defensible range: 6.4e10 to 9.1e10 active parameters.
> 4. **An entire artefact never opened.** `grep -i 'substrate b|psychohistory|validation/'` over this document returns zero matches. The psychohistory validation suite is the adjudicator that measurement F9 depends on, and this document never opened it. It carries CRITICAL and HIGH defects of its own.
>
> The findings F-01, F-03, F-06 and F-07 stand as real fixes. Severity labels that overstated what `logos.tex` delivers have been re-graded in place, and every correction is attributed inline to its finding ID.

**Audit date:** 2026-07-25. Organised by severity, not by section order. Severity here is engineering priority: how much of the architecture moves if the finding holds.
...
| F-04 | BAR extrapolated 400× with no hedge | **HIGH**, unresolvable by argument | **Named, still unhedged**, and now entangled. §**3.4** (`sec:extrapolation`, renumbered from §3.3 when §3.3 became the lineage section), measurement F2. The central bet. §3.3 adds what F-04 did not see: BAR is also what forces lineage sharing to `λ ≥ 1`, so F-04, **X-04** and **C-02** are three views of one parameter (`GAPS.md` §4a). Dropping BAR to rescue the diversity claim removes the method F-04 is the extrapolation of |
...
| F-07 | Latent dispatch claimed as confidentiality protection | **HIGH** | **Dropped**, §9.1 Remark 5 |
| F-08 | Canary AUROC 1.0 generalised beyond its adversary model | **HIGH** | **Bounded**, §10.1 + Conjecture 2, measurement F8 |
| F-09 | Master router is an unanalysed single point of failure | **MEDIUM** | **Designed, untested**, §4. Bandit training signal, verifiability asymmetry, failure ladder, descriptor warm-start, post-routing mask. §4.3 states only that the warm start is *measurable* at 1B scale, so router drift under tower swap is deferred to measurement F10, not closed (X-07) |
| F-10 | No fault tolerance or straggler analysis anywhere | **MEDIUM** | **Solved for three of four paths**, §11.3: tower unreachable, sticky owner lost, straggler in expert dispatch. The fourth gap named in the F-10 body below, the FaaSMoE cold-start budget, is not in §11.3; `logos.tex:438` defers it to measurement F7 instead (X-17) |
| F-11 | Truncated and reconstructed formulas | **MEDIUM** | **Labelled as reconstructions**, §5.3 Remark 3 and §9.2 |
| F-12 | No cost model, no evaluation method for a composed ensemble | **MEDIUM** | **Solved for single-tower serving; the §12 loop workload is unpriced** (X-06). §11.1 now gives a bracket of 106 to 320 accelerators and three traffic cases rather than one, and §11.2 **retires η**: the metric is **AIQ** (RouterBench) as the primary scalar with **APGR** and **CPT(50%)** (RouteLLM) as the secondary pair, all adopted from published work and claimed as no contribution. The η this row used to name is superseded; see `GAPS.md` §3 |
...
| F-14 | Domain partition asserted, never justified | **LOW-MED** | **Criterion supplied, second axis added, partition still unmeasured**, §**3.5** (`sec:partition`, renumbered from §3.4). A three-axis criterion (corpus disjointness, objective conflict, update cadence) which says **four towers, not five**, Mathematics and Logic failing all three. §3.5 now also states that the partition is **two-dimensional, domain crossed with size**, and that the two axes have opposite properties on every quantity that matters: on the domain axis lineage is low and there is **no key-value reuse across domains**, because towers that never shared a basis emit incommensurable keys and values; on the size axis the arrangement is a distillation ladder, lineage is shared by construction and **key-value reuse works**, blocked by layer-count correspondence and separately-learned projections rather than by tensor shape (grouped-query families hold `head_dim` and `num_kv_heads` fixed across sizes, so at 8 KV heads and head dimension 128 one layer's per-token KV state is 2 × 8 × 128 = 2048 elements at every tier). Cost of the size axis: a 7B and a 70B tier beside a 2.8T parent is 7.7e10 parameters, **2.75 percent** of the tower, **40.9 GB** at MXFP4's true 4.25 bits, and 1.9 to 2.7 percent of the tower's training FLOPs. **The four-towers verdict survives the second axis unchanged**, because all three criteria are stated on the domain axis; what the size axis changes is the served-artifact count, which multiplies evaluation arms and is one reason §11.2 needs a metric that takes no maximum over arms. F-14's actual complaint, no ablation and no clustering evidence, is **still unanswered**: an argued partition is not a measured one, §3.5 itself closes with "a data question we can specify but have not run", and Table 1 still prints five (X-17). **The measurement now has an F-number: it is measurement F11 and it needs no accelerator.** The grouped-query configuration is stated in the paper from memory of published model cards and flagged there for verification in `PRIOR_ART_v03.md`; the argument degrades to "shapes may need aligning too" if it is wrong |
| F-15 | Prop. 2 requires overlapping corpora while §13 treats towers as clean residency partitions | **MEDIUM**, found late | **Contradiction resolved, consequence unquantified**, §11.4. Split by residency class, and the headroom "shrinks" by an amount §11.4 concedes it does not bound. Let `f` be the residency-bound corpus fraction: the system needs `5.6e13 × (1 + 4f)` unique tokens, so at `f = 1` the headroom is exactly zero. §11.4 now reads that against the token-supply estimate: against the central unique-stock figure of about 6e13 the requirement is already at the estimate at `f = 0` and exceeds it above **`f ≈ 0.02`**; against the 90-percent upper bound of 2e14 it survives to `f ≈ 0.64`. **`f` is no longer bounded nowhere:** §3.3 gives `f ≤ 1 - g` with `g` the pre-branch fraction of each tower's budget, and `g = 0.98` delivers exactly the threshold, at the price of the diversity claim. **Measurement is now measurement F12, needing no accelerator** (X-04, X-17) |

**F-04, F-13, F-14 and F-15 are not closed, and F-09 and F-10 are closed only in part.** F-04 cannot be closed by argument, only by a training run. F-14 and F-15 need measurements, corpus overlap and residency fraction respectively.

> **Corrected, and the correction runs in the paper's favour.** This paragraph used to end "that no measurement in the §15 table covers". That is false as of the current `logos.tex`: §15 now carries **fourteen** measurements, and the two measurements F-14 and F-15 are open on are **F11** (corpus overlap) and **F12** (residency fraction), both marked as needing **no accelerator at all**. What remains true is that neither has been run, so F-14 and F-15 stay open. The other two additions are **F13** (whether corpus-level difference is exploitable by debate) and **F14** (whether a common-seed divergent branch preserves that difference), and F14 is the measurement §3.3 minted for the lineage parameter that F-04 above is now entangled with. Of the fourteen, **seven run on one consumer accelerator** (F3, F4, F5, F9, F10, F13 and F14), and **two need none**, F11 and F12.
...
**Fixed:** §2.3, Proposition 2. Made measurement **F1**.
...
**Status:** cannot be resolved by argument. Named as measurement **F2**, BLOCKED on training compute. This is the architecture's central bet and it is unhedged.

> **What this finding did not see, added from `logos.tex` §3.3.** F-04 treats BAR as a method whose only open question is scale. It is also the thing that fixes a parameter this review never named: **how much lineage towers share.** Write `λ = 2` for a shared corpus, `λ = 1` for divergent branches of one seed (which is Branch-Train-MiX as published), `λ = 0` for independent initialisations and corpora. BAR joins separately post-trained experts into one mixture-of-experts and the BTM merge averages shared attention weights across branches, and neither operation means anything across independently initialised models whose neurons sit in unrelated permutations. **So BAR requires `λ ≥ 1`, and the architecture cannot drop BAR.** The consequence is that "independently trained towers" weakens to "one common seed, five divergent continued-pretraining branches", and the diversity conjecture (which F13 states at `λ = 0`, explicitly excluding two finetunes of one checkpoint) is being asserted about an object the reference architecture cannot build. Whether a branch is a large enough perturbation is **measurement F14**, on the same instrument as F13's limb (a), and the paper does not resolve it by argument.
>
> The arithmetic runs the other way and it is the same knob. With `g` the pre-branch fraction of each tower's token budget, the seed corpus is shared by construction, so F-15's `f` obeys `f ≤ 1 - g` and the unique-supply requirement is bounded by `5.6e13 (1 + 4f) ≤ 5.6e13 (5 - 4g)`, which at `g = 0.98` is **6.05e13**, exactly the shared-core threshold §11.4 needs against the central token-supply estimate, and at `g = 0` is the undecomposed **2.8e14**. **The same knob, turned the same way, buys the data ceiling and costs the diversity.** So F-04, F-15 and C-02 are three views of one parameter, not three findings, and no setting of `g` satisfies all three. The full record is `GAPS.md` §4a. Two things the paper marks rather than asserts, and this document does not upgrade them: whether BAR *strictly* requires a common seed, as opposed to being demonstrated on one, is a reading of its source; the same question for the BTM attention-averaging step is a stronger inference, believed sound because averaging weights across unaligned bases is a known failure.
...
**Fixed:** §8.3, made measurement **F5**. The check that matters is on *real* hidden states, which the harness of §12 produces as a by-product; a synthetic-latent version would be circular.
...
**Fixed:** §9.1 Remark 5. Benefit retained, confidentiality claim dropped.
...
**Status:** measurement **F8**. Needs a benign null *measured* across two accelerators of different models; a matched pair only reproduces the published setting. **We expect this one to fire against our own architecture.**
...
> **Status against `logos.tex` §3.5, which is where this now lives.** The finding as written is answered in one direction and unanswered in the other. Answered: there is now a criterion, three axes, and applying it returns **four towers, not five**, with Administration separating cleanly on all three and Mathematics-and-Logic merging. Unanswered: the criterion is argued, not measured, and Table 1 still prints five. §3.5 also adds a **second axis, domain crossed with size**, which this finding did not anticipate and which is not symmetric with the first: domain towers are peers with low lineage and no cross-domain key-value reuse, while the size axis is a distillation ladder with shared lineage where key-value reuse works. That second axis is cheap (2.75 percent of the tower's parameters, 40.9 GB, 1.9 to 2.7 percent of its training FLOPs) and **does not disturb the four-towers verdict**, because all three criteria are properties of what a tower is trained on and for, not of how large it is. What it does disturb is evaluation: more arms, and any metric taking a maximum over arms degrades as the count grows, which is part of why §11.2 dropped η. The measurement this finding actually asked for is now **measurement F11**, and it needs no accelerator.

---

## 4. What is genuinely good about the architecture

A list of only faults is not useful. These parts hold up:
...
**A sixth step has since been added by the paper and this document did not see it coming.** `logos.tex` §3.3 names **lineage sharing** as an explicit design parameter and shows that steps 3 and 5 above are competing for the same knob. Step 3's peak-unique-corpus saving survives residency constraints only when the shared core is above about 98 percent of each tower's corpus; the informative-difference argument, which F-04's body and C-02 both bear on, needs the towers *not* to share a corpus. Branch-Adapt-Route settles it at `λ = 1` because the architecture cannot drop the method, so the corrected spine ends:

6. **Every tower is a divergent continued-pretraining branch of one common seed, not an independent model**, and the paper's three tower motivations (update economics, alignment isolation, informative difference) are priced against that weaker object rather than the stronger one the abstract used to imply. Whether the weaker object still carries the diversity conjecture is **measurement F14**, it runs on one consumer accelerator, and it has not been run.

This is a still less dramatic story and a still truer one. The full arithmetic, and the collapse of F-04, F-15 and C-02 into one parameter, is `GAPS.md` §4a.
