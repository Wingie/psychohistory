...
**Audit date:** 2026-07-25. **Stance:** adversarial. This document is the referee report the paper would get. It is organised by severity, not by section order.

The short version: **the architecture is buildable and the individual mechanisms are real, but three of its load-bearing arguments are arithmetically or logically wrong as originally stated, and two components are in the wrong place.** All five are fixable, and the corrected versions are in `logos.tex`. The single biggest *unfixable-by-argument* risk is the BAR extrapolation (§2.1).

---

## 0. Severity index

| # | Finding | Severity | Fixed in paper? |
|---|---|---|---|
| F-01 | Dense compute arithmetic applied to sparse towers — a 55× error | **CRITICAL** | Yes, §2.2 |
| F-02 | Token fragmentation argument is arithmetically false as stated | **CRITICAL** | Yes, §2.3 (Prop. 2) |
| F-03 | Regulatory argument built on the wrong FLOP number | **CRITICAL** | Yes, §10.2 — turned into a contribution |
| F-04 | BAR extrapolated 400× with no hedge | **HIGH** (unresolvable by argument) | Named as F2, unhedged |
| F-05 | 10T vs 14T slides between sections | **HIGH** (it breaks F-03) | Yes, §2.4 |
| F-06 | RQ-VAE codebook placed in the hidden-state path | **HIGH** | Yes, §7.3 — relocated |
| F-07 | Latent dispatch claimed as confidentiality protection | **HIGH** | Yes, Remark 6 — withdrawn |
| F-08 | Canary AUROC 1.0 generalised beyond its adversary model | **HIGH** | Yes, §9.1 + Conjecture 2 |
| F-09 | Master router is an unanalysed single point of failure | **MEDIUM** | Named, not solved |
| F-10 | No fault tolerance / straggler analysis anywhere | **MEDIUM** | Named as a gap |
| F-11 | Truncated / invented formulas | **MEDIUM** | Yes, §8.2 flagged as reconstruction |
| F-12 | No cost model, no eval methodology for a composed ensemble | **MEDIUM** | Named as a gap |
| F-13 | Petals analogy imported without checking it transfers to MoE | **LOW-MED** | Partially |
| F-14 | Domain partition (Code/Bio/Math/Logic/Admin) asserted, never justified | **LOW-MED** | Named |
...
**Fixed:** `logos.tex` §2.2, Proposition 1, and Table 1 now separate the estimators. Reproduced numerically in `validation/L1_scaling_arithmetic/`.
...
**Fixed and promoted:** §10.2 and Conjecture 2 (*modular composition is a compute-threshold arbitrage surface*). This is now the paper's strongest original contribution — it came out of chasing down a wrong number.
...
**Fixed:** §7.3, made falsifier **F5**, mechanism measured in `validation/L5_dcvq_collapse/`.
...
**Fixed:** Remark 6 — benefit retained, confidentiality claim explicitly withdrawn.
...
2. **Numerics heterogeneity.** The benign null is set by hardware drift. A network deliberately spanning heterogeneous accelerators, **mixed MXFP4/NVFP4 quantization (this paper's own §6)**, and independently compiled kernels has a null that is wider, multi-modal, and node-dependent. A single pooled null then over-covers the tampering signal. Note this architecture *creates* the problem in §6 and then relies on a detector that assumes it away in §9.
...
**Status:** falsifier **F8**, tested in `validation/L6_canary_integrity/`. **We expect this one to fire against our own architecture.**
...
- **Adversarial routing.** A user who can shape which tower serves them can steer around the Administration/safety tower. This is the same object as the compliance mask in §10.3, operated by the wrong party.

**Status:** named in `logos.tex` §12 ("not addressed at all"), not solved. Should be P1 in `GAPS.md`.
