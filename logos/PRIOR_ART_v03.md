...
That last detail is the most important empirical datum in this entire section, and it cuts against the proposal. Even between models of *identical architecture and identical size* differing only by fine-tuning, direct cache reuse was not good enough, and per-layer selective recomputation was required. The proposed mechanism asks for direct reuse across a four-times depth difference. DroidSpeak is the closest thing to a negative result the literature offers, and v0.3 must confront it rather than cite it as support.
...
Across the eleven search strings in §6.1, plus four arXiv API queries, **no paper was found that trains a family of siblings of differing depth from one parent under an explicit objective that makes their key and value projections interchangeable, such that a KV cache prefilled by a smaller sibling is consumed directly by a larger sibling with no translation adapter and no re-prefill.**

The nearest published things, in decreasing order of proximity:

1. ICaRus / PrefillShare: interchangeability achieved by sharing one frozen KV-producing module, not by aligning independent ones.
2. C2C / LCF / SCD: interchangeability across genuinely different models, achieved by a trained translation adapter at serving time.
3. MatFormer: attention projections identical across the ladder by construction, KV compatibility never claimed or measured.
...
6. DroidSpeak: direct cross-model reuse attempted at same architecture and same size, found to require per-layer selective recomputation.

**Verdict: candidate contribution, genuinely unclaimed as stated.** With three conditions on how it is written.

- It must cite and differentiate all seven papers above. A referee in KV-cache systems will know at least four of them, and ICaRus plus PrefillShare are recent enough that "unaware" is the charitable reading and "non-disclosing" is the other one. This is the exact shape of C-04.
- It must confront DroidSpeak rather than cite it as support. DroidSpeak is evidence that the alignment problem is harder than shape compatibility implies, at zero depth difference.
- It must state the layer-correspondence problem explicitly. The enabling fact gives per-layer shape identity; it gives no answer to which of the small sibling's 32 layers supplies KV to which of the large sibling's 126, and the paper's own falsifier for this mechanism should be stated in those terms.
...
| Task class | Looped 12⊗2 | iso-FLOP baseline | Fraction of gap to 24-layer covered |
|---|---|---|---|
| Closed Book QA (memorization) | 9.3% | 11.2% | 34% |
| Math Word Problems (reasoning) | 34.3% | 26.7% | 282% (24-layer reference: 29.3%) |
| Reasoning Primitives | 51.2% | 35.7% | (outperforms across all budgets) |

The looped model is **worse than its own iso-FLOP baseline on closed-book QA** and beats even the deeper non-looped model on math word problems. That is the distinction, measured, seventeen months ago.
...
|---|---|---|
| (1) Denominator zero when best single tower dominates | **Avoided outright.** AIQ has no ratio and no oracle in it. It is a normalized integral of quality over cost. A dominating single model simply produces a flat curve with a well-defined area. | **Not avoided structurally.** The denominator `r(M_s) - r(M_w)` is zero if the designated weak model matches the strong one. But the endpoints are two models designated *a priori*, not a post-hoc max over k, so the condition is diagnosable before evaluation rather than emerging from per-domain slicing. Strictly weaker failure, same shape. |
...
**1. KV-cache-aligned distillation across a size ladder.** Substantial residual novelty, and the only one of the four with any. Not found as stated across the most thorough search of the four. Seven adjacent papers exist and must be cited; two of them (ICaRus, PrefillShare) get the same interchangeability property by a cheaper route and are four to five months old; one (DroidSpeak) is evidence the alignment is hard at zero depth difference; one (MatFormer) already has the property latent and unexploited. It is the highest-value item and also the one carrying the largest C-04 exposure, because the subfield is crowded and moving fast.
...
Twenty-one fetches. Every claim in §§1-5 traces to one of these.
