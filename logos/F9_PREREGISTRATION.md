...
1. **The F9 total is no longer a single number.** Generation, the `tau_JS` pool, S4, S5 and F13 limb (b) all run on frozen open-weight proposers, and their cost is linear in the proposers' parameter count. At 0.5B-class proposers the F9 total is **1,376.8 GPU-h**, at 1B-class **1,536.5**, at 8B-class **3,773.3** (§8.1). **The roster is not frozen in this document**, so the honest statement of the F9 rung is a band with a named planning instantiation, not a total. What settles it: freezing `proposers/roster.yaml` and measuring quantized proposer inference in the day-one probe.
2. **Generation is now the second largest line and it is bigger than it was.** It moves from 137.9 to 281.3 GPU-h at the planning instantiation (it stood at 393.9 until Study 2's forbidden per-seed corpus multiplier was withdrawn, §8.1). The old figure was cheap because it was priced against proposers that would have had to be trained first, out of a budget that never paid for them, and that would have been the homogeneous pair finding C-02 says returns a martingale. **The old number bought an instrument that does not exist; the new one buys an instrument that does.**
...
| Same, before Study 2's per-seed corpus multiplier and duplicated A0 arm were withdrawn | 1,683.6 | $337 to $421 rented | 589 kWh, ~EUR 177 |
...
python3: generation GPU-h = 9.67742e9 / throughput(N) / 3600
         ...
         (superseded, at the withdrawn 14,112,903-proposal volume: 196.9 / 393.9 / 590.8 /
...
python3: Study 1: 4032258*960/9555/3600 = 112.5 GPU-h
         Study 2: 2016130*960/9555/3600 =  56.3 GPU-h   (was 168.8 under the withdrawn 3x)
...
- **M = 2,000 items**, drawn from the §3 battery in the proposer rendering with RNG seed 20260725, so the sample is reproducible. **Tense corrected:** an earlier revision said "the items are the ones already sealed", and nothing is sealed. §0 of this document says "nothing in this document has been lodged", every seal row in §11 reads *(fill at lodge)*, and `logos-harness/` does not exist in this repository. The battery **will be sealed at §11 before this limb runs**.
